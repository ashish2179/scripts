...
if(len(user)>2):
    ...
    raw_date = str(x.strftime("%y"))+'/'+str(x.strftime("%m"))+'/'+str(x.strftime("%d"))+','+str(x.strftime("%H"))+':'+str(x.strftime("%M"))+':'+str(x.strftime("%S"))+'.'+str(x.strftime("%f"))
    raw_data.append({
                        'DT': raw_date,
                        ...
                        str(int(round(random.uniform(nrml_pressure_max,nrml_pressure_max*2), 2)*100)),
    ...
    if(len(data)==1):
        ...
        try: 
            ...
            client.send_message(rphone, 'Device send data api error', parse_mode='html') 
    ...
    try: 
        ...
        # sending message using telegram client 
        client.send_message(rphone, 'Device logging api error', parse_mode='html') 
    except Exception as e: 
